...
let backoffUntil=0;
async function evaluate(apiKey:string,text:string,rules:string[],threshold:number){
  const res=await fetch('https://api.typesafe.ai/v1/systemone',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify(requestFor(text,rules)),signal:AbortSignal.timeout(15000)});
  ...
  return decision((await res.json()).answers,rules,threshold);
}
...
chrome.runtime.onMessage.addListener((msg,sender,reply)=>{
  ...
  if(msg?.type==='recordHidden'){
    const job=historyChain.then(async()=>{
      ...
      const entry={text:msg.text.slice(0,240),rule:msg.rule.slice(0,200),score:typeof msg.score==='number'&&Number.isFinite(msg.score)?Math.max(0,Math.min(1,msg.score)):0,url,site,hiddenAt:Date.now()};
      const old=Array.isArray(hiddenPosts)?hiddenPosts:[];
      await chrome.storage.local.set({hiddenPosts:[entry,...old.filter(p=>url?p.url!==url:p.text!==entry.text||p.site!==site)].slice(0,50)});
  ...
  const run=async()=>{
    ...
    let result;
    try{result=await evaluate(apiKey,msg.text,config.rules,config.threshold);}catch(e){backoffUntil=Date.now()+60000;throw e;}
