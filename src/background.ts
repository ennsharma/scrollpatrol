...
chrome.runtime.onMessage.addListener((msg,sender,reply)=>{
  ...
  const run=async()=>{
    ...
    if(!apiKey) throw new Error('Add your Jev API key in Scrollpatrol.');
    ...
    if(!res.ok){backoffUntil=Date.now()+60000;throw new Error(res.status===401?'Invalid Jev API key. Update it in Scrollpatrol.':`Jev unavailable (${res.status}). Posts remain visible.`);}
