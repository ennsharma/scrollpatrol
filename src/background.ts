...
let backoffUntil=0;
async function evaluate(apiKey:string,text:string|PostContext,rules:string[],threshold:number){
...
chrome.runtime.onMessage.addListener((msg,sender,reply)=>{
  ...
  const run=async()=>{
    ...
    const key=JSON.stringify([post,config.rules,config.threshold]);
    if(cache.has(key)) return cache.get(key);
    if(Date.now()<backoffUntil) throw new Error('API paused after an error. Try again shortly.');
    let result;
    try{result=await evaluate(apiKey,post,config.rules,config.threshold);}catch(e){backoffUntil=Date.now()+60000;throw e;}
