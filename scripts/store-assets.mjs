...
const profile=await mkdtemp(resolve(tmpdir(),'scrollsafe-store-'));
...
try{
 ...
 await page.evaluate(()=>{const aside=document.createElement('aside');aside.className='store-copy';aside.innerHTML='<h2>A little less<br>internet.</h2><p>Describe what you’re tired of seeing.<br>Scrollsafe mutes matching feed posts.</p><p>Natural-language rules.<br>Reversible mutes. Your choice.</p><small>Open source · Bring your own TypeSafe API key<br>Provider usage billed separately</small>';document.body.append(aside);});
 await mkdir('store/assets',{recursive:true});
 await page.screenshot({path:'store/assets/screenshot-1280x800.png'});
 await page.setViewportSize({width:440,height:280});
 await page.evaluate(()=>{document.body.innerHTML='<img src="brand/scrollsafe-wordmark.png" alt="Scrollsafe"><p>A little less internet.</p>';document.head.querySelectorAll('style,link').forEach(e=>e.remove());});
